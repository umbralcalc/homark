...
- **Deterministic grid calibration:** [`cmd/calibratespine`](cmd/calibratespine/main.go) searches `bank_beta` and optional grids for `price_drift`, `supply_beta`, and **`demand_supply_pressure`** (`-demand-supply-steps`, `-demand-supply-beta-lo/hi`) to minimise RMSE of **log price** vs a filled historical series (`ForwardFillAffordableFields` + `MonthlyLogSeries`), with optional **joint** weight on log-earnings RMSE (`-w-log-earnings`). **Evolution Strategy** (`-es-steps N`) returns `ThetaMean` / `ThetaCov`; **`-es-json-out path`** writes them for [`cmd/policyscenario`](cmd/policyscenario/main.go). Uses zero diffusion for a smooth objective; re-enable diffusion when running `forwardspine` with the printed coefficients.
- **Data / model credibility:** [`cmd/credibilityreport`](cmd/credibilityreport/main.go) prints **log PPD vs log HPI** correlations (all sales and D/S/T/F), a **permissions→completions** lag scan (`-lag-max`), and optional **`-validate-months N`** train/test calibration stats using the same grid flags as `calibratespine`. Helpers live in [`pkg/housing/credibility.go`](pkg/housing/credibility.go).
- **Policy scenario grid (Phase 4 MVP):** [`cmd/policyscenario`](cmd/policyscenario/main.go) runs a **deterministic** forward spine over a Cartesian grid of **`-approvals`** (pipeline inflow rates), **`-bank-scales`** (multipliers on historical `bank_rate_pct`), and optional **`-posterior-samples`** draws from **`theta_cov`** in a JSON file produced by **`calibratespine -es-json-out`**. CSV to stdout: mean and terminal **affordability** (`P/E`) per cell. Without **`-posterior`**, the six theta components come from the same flags as **`forwardspine`** (`-bank-beta`, `-price-drift`, …).
...
**Still to do (see phases below):** richer Phase 4 (density/tenure strategies, charts, write-up); multi-LA and extensions (Phase 5).
...
4. **Credibility (optional):** with PPD + completions on the spine, `go run ./cmd/credibilityreport -la "Leeds"` prints PPD–HPI correlations and a permissions–completions lag scan; add **`-validate-months 12`** for train/test RMSE like `calibratespine`.
5. **Calibrate:** grid search via [`scripts/calibrate_pilot_example.sh`](scripts/calibrate_pilot_example.sh), or ES: `go run ./cmd/calibratespine -la "Leeds" -es-steps 400 -es-json-out posteriors/leeds.json`, then `forwardspine` or **`go run ./cmd/policyscenario -la "Leeds" -posterior posteriors/leeds.json -approvals 0,80,160 -bank-scales 1,1.1`** (add **`-posterior-samples 50`** to propagate ES uncertainty). If the spine has **no** earnings or ONS ratio columns, calibration still runs by imputing log earnings from **`-init-median-ratio-fallback`** (default 7) on those months—replace with real enrichment when you can.
...
|-------|-------------|-----------------|-------------------------|
| **1 — Data** | Ingest transactions, affordability, supply, context | **UK HPI + BoE + Table 122** via [`cmd/fetchspine`](cmd/fetchspine/main.go); **optional** ONS affordability + ASHE-style earnings CSVs, PPD+NSPL (**typed** PPD medians/counts on spine when aggregated by LR type), **permissions**, **completions** (`completions_approx_monthly`, `-completions`), URLs on CLI; **five pilot LAs** in [`pkg/ladata/targets.yaml`](pkg/ladata/targets.yaml); [`cmd/spinehealth`](cmd/spinehealth/main.go) + [`scripts/spinehealth_gate.sh`](scripts/spinehealth_gate.sh) for pay/ratio coverage | Optional expansion past five pilots; richer supply series beyond Table 122 |
...
| **3 — Learning** | SBI / fitted dynamics from data | **Complete** for single-LA grid/ES calibration. [`cmd/calibratespine`](cmd/calibratespine/main.go): grid search + **ES** (`-es-steps N`), **`-es-json-out`**, **`-w-log-earnings`**, **`-validate-months`**, **`-laplace`**, PPD-aware targets. **Credibility:** [`cmd/credibilityreport`](cmd/credibilityreport/main.go) + [`pkg/housing/credibility.go`](pkg/housing/credibility.go) (PPD–HPI correlations, permissions–completions lag, optional hold-out stats). | Multi-LA coupling (Phase 5 item) |
| **4 — Decision science** | Policy strategies and scenarios | **MVP:** [`cmd/policyscenario`](cmd/policyscenario/main.go) — approval-rate grid, bank-path scaling, optional `ThetaCov` samples; CSV affordability summaries | Density/tenure action sets, narrative outputs, visualisations, full scenario library |
...
**Homark status:** **partial.** [`cmd/policyscenario`](cmd/policyscenario/main.go) implements a first **scenario runner** (approval-rate × bank-scale × posterior sampling) with deterministic forward paths and CSV output. The design sections below remain the **target** for richer strategies (density, tenure, composition metrics) and reporting. Export **`calibratespine -es-json-out`** then pass **`policyscenario -posterior`** for uncertainty-aware grids.
...
- [x] **Pipeline stock** (deterministic and stochastic paths, permissions column on spine, attrition on stochastic path) — see `PipelineStockValuesFunction`, `StochasticPipelineIteration`, and `PriceDriftValuesFunction` in `pkg/housing`
...
- [x] **Minimal policy scenario grid:** [`cmd/policyscenario`](cmd/policyscenario/main.go) — `-approvals`, `-bank-scales`, `-posterior` + `-posterior-samples` (ES JSON from **`-es-json-out`**)
- [ ] Implement richer planning strategies as action sets (density mix, tenure mix beyond approval-rate scaling)
