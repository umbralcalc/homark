...
- **Spine-informed forward simulation:** [`cmd/forwardspine`](cmd/forwardspine/main.go) uses [`pkg/housing.ForwardSpineConfigs`](pkg/housing/forward_spine.go): historical `bank_rate_pct` is streamed from storage into [`DriftDiffusionBankChannelIteration`](pkg/housing/drift_diffusion_bank.go) (log-price drift includes `bank_beta × (rate/100)`); log earnings use constant-coefficient `DriftDiffusionIteration`; initial log levels come from the **first** spine month (not every row); if that month has no `median_gross_annual_pay` or `median_ratio` (common before ONS series start), implied log earnings uses `-init-median-ratio-fallback` (default 7). Coefficients align with [`cfg/single_la_housing.yaml`](cfg/single_la_housing.yaml) defaults; tune with `-bank-beta`, `-price-drift`, etc.

**Still to do (see phases below):** calibration / SBI for coefficients, richer covariates (e.g. supply), and the decision-science policy layer.
...
- [x] **Forward** monthly sim with spine **bank rate** as exogenous input to log-price drift (`cmd/forwardspine`, `pkg/housing.ForwardSpineConfigs`)
