...
- **Spine-informed forward simulation:** [`cmd/forwardspine`](cmd/forwardspine/main.go) uses [`pkg/housing.ForwardSpineConfigs`](pkg/housing/forward_spine.go): **`bank_rate`** and **`supply_net`** from `FromStorageIteration`; **`pipeline`** and **`price_drift`** use [`ValuesFunctionIteration`](https://github.com/umbralcalc/stochadex) via [`PipelineStockValuesFunction` / `PriceDriftValuesFunction`](pkg/housing/forward_values.go) (drift = base + bank + supply/scale − pipeline dampening); **`log_price`** is [`DriftDiffusionIteration`](https://github.com/umbralcalc/stochadex) with `drift_coefficients` wired from `price_drift`. Log earnings use `DriftDiffusionIteration`. Initial log levels use only the **first** spine month; `-init-median-ratio-fallback` fills missing early ONS fields. Flags: `-supply-beta`, `-pipeline-beta`, `-approval-rate`, etc.
- **Deterministic grid calibration:** [`cmd/calibratespine`](cmd/calibratespine/main.go) searches `bank_beta` (and optional `price_drift`, `supply_beta` grids) to minimise RMSE of **log price** vs a filled historical series from the spine (`ForwardFillAffordableFields` + `MonthlyLogSeries`). Uses zero diffusion for a smooth objective; add stochastic diffusion again when running `forwardspine` with the printed coefficients.
